...
export const Notebook: React.FC<Props> = ({
  ...
  // Stale when SQL drifted OR the session data epoch moved past this result
  // (latest-data wins: keep showing a badge, never silently reuse).
  const staleById = useMemo(() => {
    const m: Record<string, boolean> = {};
    for (const c of cells) {
      if (c.type !== "sql" || !c.ranOnce) continue;
      ...
      const dataDrift =
        c.resultId != null &&
        (c.ranDataEpoch == null || c.ranDataEpoch !== dataEpoch);
      m[c.id] = !!(sqlDrift || dataDrift);
    }
    return m;
  }, [cells, compiledById, dataEpoch]);
  ...
  // Stop a Run-all sweep: halt the loop (cancelAllRef) and cancel any Journal
  // cell that is currently in flight. Scoped to Journal only — do not broadcast
  // a global run cancel that would stop IDE / NodeFlow / Field Explorer / etc.
  const cancelAll = () => {
                  ...
                  ? cells
                      ...
                      .filter(
                        (s) =>
                          ...
                          s.ranOnce &&
                          s.resultId &&
                          // Latest-data wins: charts/pivots only bind fresh sources.
                          !staleById[s.id],
                      )
                      .map((s) => ({
